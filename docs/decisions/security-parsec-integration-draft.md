...
| File | Change |
|---|---|
| `cmd/satellite/main.go` | `ParsecEnabled`/`ParsecSocketPath` in `SatelliteOptions`; new flags; `MustDetect()` startup check |
...
1. **Wire `KeyProvider` into `pkg/config/manager.go`**
   ...
   variant that accepts a `crypto.Provider` so the caller (in `cmd/satellite/main.go`) can pass a
