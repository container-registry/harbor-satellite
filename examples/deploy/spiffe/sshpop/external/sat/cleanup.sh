...
for SAT in edge-01 edge-02; do
    echo "> Deleting SPIRE entry for ${SAT}..."
    ENTRY_ID=$(docker exec spire-server /opt/spire/bin/spire-server entry show \
        -spiffeID "spiffe://harbor-satellite.local/satellite/region/default/${SAT}" \
...
# Include the edge-02 override so its containers and volumes are removed too.
echo "> docker compose down -v --remove-orphans"
docker compose -f docker-compose.yml -f docker-compose.edge-02.yml down -v --remove-orphans

# Delete robot account from Harbor
HARBOR_URL="${HARBOR_URL:-http://localhost:8080}"
HARBOR_USERNAME="${HARBOR_USERNAME:-admin}"
HARBOR_PASSWORD="${HARBOR_PASSWORD:-Harbor12345}"
for SATELLITE_NAME in ${SATELLITE_NAME:-edge-01 edge-02}; do
