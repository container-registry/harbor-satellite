...
// NewServer creates a new api groundcontrol server but does not configure it
...
// Server for the groundcontrol API
type Server struct {
	...
	SocketPath    flags.Filename `long:"socket-path" description:"the unix socket to listen on" default:"/var/run/groundcontrol.sock"`
...
func (s *Server) Serve() (err error) {
	...
	if s.hasScheme(schemeUnix) {
		...
		wg.Add(1)
		s.Logf("Serving groundcontrol at unix://%s", s.SocketPath)
		go func(l net.Listener) {
			defer wg.Done()
			if errServe := domainSocket.Serve(l); errServe != nil && !errors.Is(errServe, http.ErrServerClosed) {
				s.Fatalf("%v", errServe)
			}
			s.Logf("Stopped serving groundcontrol at unix://%s", s.SocketPath)
		}(s.domainSocketL)
	}
	...
	if s.hasScheme(schemeHTTP) {
		...
		wg.Add(1)
		s.Logf("Serving groundcontrol at http://%s", s.httpServerL.Addr())
		go func(l net.Listener) {
			defer wg.Done()
			if errServe := httpServer.Serve(l); errServe != nil && !errors.Is(errServe, http.ErrServerClosed) {
				s.Fatalf("%v", errServe)
			}
			s.Logf("Stopped serving groundcontrol at http://%s", l.Addr())
		}(s.httpServerL)
	}
	...
	if s.hasScheme(schemeHTTPS) {
		...
		wg.Add(1)
		s.Logf("Serving groundcontrol at https://%s", s.httpsServerL.Addr())
		go func(l net.Listener) {
			defer wg.Done()
			if errServe := httpsServer.Serve(l); errServe != nil && !errors.Is(errServe, http.ErrServerClosed) {
				s.Fatalf("%v", errServe)
			}
			s.Logf("Stopped serving groundcontrol at https://%s", l.Addr())
