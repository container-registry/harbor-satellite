...
const removeImageFromLabel = `-- name: RemoveImageFromLabel :exec
DELETE FROM label_images
WHERE label_id = $1 AND image_id = $2
`

type RemoveImageFromLabelParams struct {
	LabelID int32
	ImageID int32
}

func (q *Queries) RemoveImageFromLabel(ctx context.Context, arg RemoveImageFromLabelParams) error {
	_, err := q.db.ExecContext(ctx, removeImageFromLabel, arg.LabelID, arg.ImageID)
	return err
}
