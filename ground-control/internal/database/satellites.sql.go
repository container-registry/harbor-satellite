...
VALUES ($1, $2, NOW(), NOW())
RETURNING id, name, created_at, updated_at, last_seen, heartbeat_interval, mode
`
...
func (q *Queries) CreateSatellite(ctx context.Context, arg CreateSatelliteParams) (Satellite, error) {
	row := q.db.QueryRowContext(ctx, createSatellite, arg.Name, arg.Mode)
	var i Satellite
	err := row.Scan(
		&i.ID,
		&i.Name,
...
SELECT id, name, created_at, updated_at, last_seen, heartbeat_interval, mode FROM satellites
WHERE id = $1 LIMIT 1
`
