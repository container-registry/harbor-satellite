...
func TestAuditLogger_WritesStructuredEvent(t *testing.T) {
	...
	require.Equal(t, string(ComponentGroundControl), e["component"])
...
func TestAuditLogger_DerivesDefaultSeverityAndOmitsEmptyFields(t *testing.T) {
	path := filepath.Join(t.TempDir(), "audit.log")
	a, err := NewAuditLogger(AuditConfig{Enabled: true, FilePath: path}, ComponentGroundControl)
	require.NoError(t, err)
...
func TestAuditLogger_DisabledIsNoOp(t *testing.T) {
	path := filepath.Join(t.TempDir(), "audit.log")
	a, err := NewAuditLogger(AuditConfig{Enabled: false, FilePath: path}, ComponentGroundControl)
	require.NoError(t, err)
...
func TestAuditLogger_EmptyPathIsNoOp(t *testing.T) {
	a, err := NewAuditLogger(AuditConfig{Enabled: true, FilePath: ""}, ComponentGroundControl)
	require.NoError(t, err)
	require.False(t, a.Enabled())
...
func TestAuditLogger_ReconfigureSwapsDestination(t *testing.T) {
	...
	a, err := NewAuditLogger(AuditConfig{Enabled: false}, ComponentGroundControl)
	require.NoError(t, err)
	require.False(t, a.Enabled())
