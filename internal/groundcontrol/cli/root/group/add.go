...
func NewAddSatelliteCommand(runtime *common.Runtime) *cobra.Command {
	var satellite string
	var name string
	cmd := &cobra.Command{
		Use:   "satellite-to-group",
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			response, err := runtime.Client().AddSatelliteToGroupWithResponse(cmd.Context(), groundcontrol.SatelliteGroupRequest{
				Satellite: satellite,
				Group:     name,
			})
			if err != nil {
