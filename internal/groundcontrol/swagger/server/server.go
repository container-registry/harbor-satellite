...
// NewServer creates a new api ground control server but does not configure it
func NewServer(api *operations.GroundControlAPI) *Server {
	s := new(Server)
...
// Server for the ground control API
type Server struct {
	...
	SocketPath    flags.Filename `long:"socket-path" description:"the unix socket to listen on" default:"/var/run/ground-control.sock"`
	...
	api          *operations.GroundControlAPI
...
func (s *Server) SetAPI(api *operations.GroundControlAPI) {
	if api == nil {
		s.api = nil
		s.handler = nil
		return
...
func (s *Server) Serve() (err error) {
	...
	if s.hasScheme(schemeUnix) {
		...
		wg.Add(1)
		s.Logf("Serving ground control at unix://%s", s.SocketPath)
		go func(l net.Listener) {
			defer wg.Done()
			if errServe := domainSocket.Serve(l); errServe != nil && !errors.Is(errServe, http.ErrServerClosed) {
				s.Fatalf("%v", errServe)
			}
			s.Logf("Stopped serving ground control at unix://%s", s.SocketPath)
		}(s.domainSocketL)
	}
	...
	if s.hasScheme(schemeHTTP) {
		...
		wg.Add(1)
		s.Logf("Serving ground control at http://%s", s.httpServerL.Addr())
		go func(l net.Listener) {
			defer wg.Done()
			if errServe := httpServer.Serve(l); errServe != nil && !errors.Is(errServe, http.ErrServerClosed) {
				s.Fatalf("%v", errServe)
			}
			s.Logf("Stopped serving ground control at http://%s", l.Addr())
		}(s.httpServerL)
	}
	...
	if s.hasScheme(schemeHTTPS) {
		...
		wg.Add(1)
		s.Logf("Serving ground control at https://%s", s.httpsServerL.Addr())
		go func(l net.Listener) {
			defer wg.Done()
			if errServe := httpsServer.Serve(l); errServe != nil && !errors.Is(errServe, http.ErrServerClosed) {
				s.Fatalf("%v", errServe)
			}
			s.Logf("Stopped serving ground control at https://%s", l.Addr())
